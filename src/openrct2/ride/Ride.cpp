...
void Ride::SetCircuitsBack(){ // will make it so we can have more than one circuit
    if (CanHaveMultipleCircuits()==false){  //can't have multiple circuits, so
        num_circuits = 1;   //reset to 1 (can only have one circuit)
    }
}
